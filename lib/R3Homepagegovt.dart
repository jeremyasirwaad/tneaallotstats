...
class R3MyHomePagegovt extends StatefulWidget {
  const R3MyHomePagegovt({Key? key}) : super(key: key);

  @override
  State<R3MyHomePagegovt> createState() => _R3MyHomePagegovtState();
}

class _R3MyHomePagegovtState extends State<R3MyHomePagegovt> {
  var agtotalcand = -1;
  var agtotalallocs = -1;
  var agtotalcols = -1;
  var agtotalfts = -1;
  ...
  Future<dynamic> fetchAlbum() async {
    final response = await http.get(
        Uri.parse('http://3.110.226.121/api/api/master/r3stcount/88888'),
        headers: {
          'x-auth-token':
...
void onSelected(BuildContext context, int item) {
  switch (item) {
    ...
    case 1:
      Navigator.of(context).push(
        MaterialPageRoute(builder: (context) => R3MyHomePagegovt()),
      );
      break;
  }
}
