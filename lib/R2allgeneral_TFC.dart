...
class R2allgeneral_TFC extends StatefulWidget {
  const R2allgeneral_TFC({Key? key}) : super(key: key);

  @override
  State<R2allgeneral_TFC> createState() => _MyHomePageState();
}

class _MyHomePageState extends State<R2allgeneral_TFC> {
  void calculatedata() {
    int anna_catg = 0;
  ...
  Future<dynamic> fetchAlbum() async {
    final response = await http.get(Uri.parse('${ip}r2stmode/88888'),
  ...
  Future<dynamic> fetchAlbumgovt() async {
    final response = await http.get(Uri.parse('${gip}r2stmode/88888'),
        headers: {'x-auth-token': token});
